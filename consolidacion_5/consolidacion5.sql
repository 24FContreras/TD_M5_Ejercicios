...
-- 1.1.1 Añadir Customer
-- este insert tiene una FK involucrada, así que usaré una dirección ya presente
INSERT INTO customer 
	   VALUES(600, 2, 'Daniel', 
			 'Harrison', 'daniel.harrison@sakilacustomer.org', 605,
			 true, '2023-04-20','2023-05-25 16:50:10.721', 1);
						   
-- 1.1.2 Modificar Customer (Update de last_update)
UPDATE customer SET last_update = NOW() WHERE customer_id = 600;

-- 1.1.3 Eliminar Customer
DELETE FROM customer WHERE customer_id = 600;

----------------------------------------------------------------
-- 1.2.1 Añadir Staff
INSERT INTO staff 
	   VALUES(3, 'Will', 'Wright', 11, 
			  'Will.Wright@sakilastaff.com',2, true, 'Will',
			  'will123456789', NOW(), NULL);
			  
-- 1.2.2 Modificar Staff (Update de last_update)
UPDATE staff SET last_update = NOW() WHERE staff_id = 3;

-- 1.2.3 Eliminar Staff
DELETE FROM staff WHERE staff_id = 3;

----------------------------------------------------------------
-- 1.3.1 Añadir Actor
INSERT INTO actor VALUES(201, 'Colin', 'Firth', NOW())

-- 1.3.2 Modificar Actor (Update de last_update)
UPDATE actor SET last_update = NOW() WHERE actor_id = 201

-- 1.3.3 Eliminar Actor
...
-- 3.1 Recuento mensual de payments
SELECT COUNT(payment_id) AS "Numero", TO_CHAR(payment_date, 'YYYY-MM') AS "Fecha", SUM(amount) AS "Total" FROM payment
